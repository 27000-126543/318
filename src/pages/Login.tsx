...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLocalError(storeError || '登录失败，请检查手机号和密码')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="flex min-h-screen">
      <div className="hidden flex-1 items-center justify-center bg-gradient-to-br from-govBlue via-govBlue/90 to-govBlue/70 lg:flex">
        <div className="relative px-12 text-white">
          ...
          <div className="relative z-10">
            ...
            <p className="mb-8 text-lg text-white/80">让政务服务触手可及</p>
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <h2 className="mb-2 font-serif text-2xl font-semibold text-gray-800">欢迎登录</h2>
          <p className="mb-8 text-sm text-gray-500">请使用手机号和密码登录</p>

          {localError && (
            <div className="mb-4 rounded-lg bg-govRed/10 px-4 py-2.5 text-sm text-govRed">{localError}</div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="flex gap-3">
                <input placeholder="请输入验证码" className="gov-input flex-1" />
                <button
                  type="button"
                  onClick={handleSendCode}
                  disabled={codeCountdown > 0 || !phone}
                  className="gov-btn-secondary shrink-0 !px-4 text-sm disabled:cursor-not-allowed disabled:opacity-50"
                >
            ...
            <button type="submit" disabled={loading || !phone || !password} className="gov-btn-primary w-full disabled:cursor-not-allowed disabled:opacity-60">
              {loading ? '登录中...' : '登录'}
            </button>
          </form>
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            还没有账号？
            <Link to="/register" className="ml-1 font-medium text-govBlue hover:underline">立即注册</Link>
          </p>
        </div>
      </div>
    </div>
